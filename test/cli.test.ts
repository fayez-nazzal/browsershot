...
import { expandOutputTemplate, outputTemplateValues, resolveOutputPath } from "../src/output-path.ts";
...
test("timestamp uses a readable sortable local date and time", () => {
  expect(timestamp(new Date(2026, 8, 5, 14, 30, 12))).toBe("2026-09-05_14-30-12");
});

test("output template values describe the resolved URL safely", () => {
  expect(outputTemplateValues("http://localhost:3000/account/settings?q=secret#panel", new Date(2026, 8, 5, 14, 30, 12))).toEqual({
    date: "2026-09-05",
    time: "14-30-12",
  ...
  expect(outputTemplateValues("https://example.com/", new Date(2026, 8, 5))).toMatchObject({ host: "example.com", route: "home" });
  expect(outputTemplateValues("file:///tmp/example.html", new Date(2026, 8, 5))).toMatchObject({ host: "file", route: "tmp-example.html" });
});

test("output templates expand known placeholders and escaped braces", () => {
  const values = outputTemplateValues("https://example.com/pricing", new Date(2026, 8, 5, 14, 30, 12));
...
test("default output paths support ergonomic groups and labels", () => {
  ...
  expect(resolveOutputPath({
    ...
    now,
  })).toBe("/repo/.browsershot/captures/example.com/account-settings_2026-09-05_14-30-12.png");
  expect(resolveOutputPath({
    capturesDirectory: "/repo/.browsershot/captures",
    url: "https://example.com/account/settings",
    group: "regression/{date}",
    label: "menu open-{time}",
    now,
  })).toBe("/repo/.browsershot/captures/regression/2026-09-05/example.com/account-settings_menu-open-14-30-12_2026-09-05_14-30-12.png");
});
...
test("structured names stay within common filesystem byte limits", () => {
  const output = resolveOutputPath({
    capturesDirectory: "/repo/.browsershot/captures",
    url: `https://example.com/${"界".repeat(200)}`,
    group: "組".repeat(200),
    label: "状".repeat(200),
    now: new Date(2026, 8, 5, 14, 30, 12),
  });
  const components = output.split("/");
  expect(components.every((component) => Buffer.byteLength(component, "utf8") <= 255)).toBe(true);
  expect(Buffer.byteLength(components.at(-1)!, "utf8")).toBeLessThanOrEqual(255);
});
